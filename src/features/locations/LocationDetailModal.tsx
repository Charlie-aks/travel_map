...
function LocationDetailContent({ onClose }: { onClose: () => void }) {
  const router = useRouter();
  const { setDetailModalOpen, selectedLocation, savedLocationIds, toggleSaveLocation } = useLocationStore();
    ...
    <>
      <div className="flex-1 overflow-y-auto overflow-x-hidden custom-scrollbar">
        ...
        <div className="relative w-full h-[400px] shrink-0">
          ...
          <button onClick={onClose} className="sm:hidden absolute top-4 right-4 z-50 w-10 h-10 flex items-center justify-center rounded-full bg-black/40 text-white hover:bg-black/60 backdrop-blur-md transition-colors border border-white/20">
            <X className="w-5 h-5" />
...
export function LocationDetailModal() {
  const { isDetailModalOpen, setDetailModalOpen, selectedLocation } = useLocationStore();
  ...
  if (isDesktop) {
    return (
      <Sheet open={isDetailModalOpen} onOpenChange={setDetailModalOpen}>
        <SheetContent side="right" className="flex flex-col w-[95vw] md:min-w-[70vw] lg:min-w-[60vw] max-w-[95vw] p-0 bg-[#f8fafc] dark:bg-slate-950 overflow-hidden border-0 rounded-l-xl shadow-2xl transition-colors" aria-describedby="dialog-description">
          ...
          <LocationDetailContent onClose={() => setDetailModalOpen(false)} />
        </SheetContent>
      </Sheet>
    );
  }
  ...
  return (
    <Drawer open={isDetailModalOpen} onOpenChange={setDetailModalOpen}>
      <DrawerContent className="bg-[#f8fafc] dark:bg-slate-950 border-t border-slate-200 dark:border-slate-800 flex flex-col p-0 overflow-hidden outline-none outline-0 max-h-[96dvh]">
        <DrawerTitle className="sr-only">{selectedLocation.name}</DrawerTitle>
        <DrawerDescription className="sr-only">{selectedLocation.description}</DrawerDescription>
        <LocationDetailContent onClose={() => setDetailModalOpen(false)} />
      </DrawerContent>
    </Drawer>
  );
